    ...
    with st.sidebar:
        ...
        if st.button("Cargar Partidos Jugados desde la API"):
            ...
            if time_elapsed < cooldown_seconds:
                ...
                st.warning(
                    f"Por favor, espera {remaining_time // 60} minutos y {remaining_time % 60} segundos antes de cargar partidos de nuevo."
                )
            else:
                with st.spinner("Cargando partidos desde la API...", show_time=True):
                    ...
                    for match in new_matches:
                        key = tuple(sorted((match["team1"], match["team2"]))) + (
                            match["winner"],
                        )
        ...
        st.dataframe(
            pd.DataFrame(
                list(data.clasification_points.items()), columns=["Equipo", "Puntos"]
            ),
            hide_index=True,
        )
        ...
        st.dataframe(
            pd.DataFrame(st.session_state.get("played_matches", [])), hide_index=True
        )

        st.subheader("📈 Resumen actual de los Grupos")
        alpha_standings, omega_standings = get_current_standings(
            st.session_state.get("played_matches", []), data.groups
        )
